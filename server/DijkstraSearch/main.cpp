...
int main(int argc, char *argv[])
{
    ...
    pthread_mutex_init(&mutex_send[2], NULL);
    pthread_mutex_init(&mutex_rec[2], NULL);
    pthread_create(&SearchThread,NULL,&search_path,NULL);
    pthread_create(&display_thread,NULL,&displayImg,NULL);//创建显示线程
    //开启接收线程
    pthread_t recv_thread,recv_thread2,recv_thread3;//存放线程id       recv_msg：线程执行的函数，将通信socket：new_socket_fd传递进去
    ...
    ret3= pthread_create(&recv_thread3,NULL,recv_msg_2,(void *) &server3.new_socket_fd);
    //开启发送线程
    ...
    ret3= pthread_create(&send_thread3,NULL,send_msg_2,(void *) &server3.new_socket_fd);
    ...
    {
        cout<<"Failed to creat server2,program exit"<<endl;
        return 0;
    }
    else
        pthread_join(send_thread2,NULL);

    if(server3.socket_fd==-1||server3.port<1025||server3.port>65535||server3.ret==-1||server3.new_socket_fd==-1||ret!=0)
    {
        cout<<"Failed to creat server3,program exit"<<endl;
        return 0;
    }
    else
        pthread_join(send_thread3,NULL);
    ...
    pthread_mutex_destroy(&mutex_rec[2]);
    pthread_mutex_destroy(&mutex_send[2]);
    return 0;
}
...
void * search_path(void *arg)
{
    ...
    //
    //Mat img = imread("/home/zwb/CLionProjects/NodeGenerator/map.pgm");
    Mat img = imread("/home/zwb/CLionProjects for three/test_map17.pgm");
    ...
    DisplayTheMap=robot[1].generateGraph().clone();
    DisplayTheMap=robot[2].generateGraph().clone();
    /*for(int i=0;i<3;i++)
    {
    ...
    while (1)
    {
        ...
        Editor[1].GetData(received_buf1);
        pthread_mutex_unlock(&mutex_rec[1]);
        pthread_mutex_lock(&mutex_rec[2]);
        Editor[2].GetData(received_buf2);
        pthread_mutex_unlock(&mutex_rec[2]);
        for(int m=0;m<=2;m++)
        {

            if(Editor[m].validData== true)
            {
                checkData=true;
                receivedFlag[m]=true;
                continue;
       ...
       // sleep(1);
        ...
        //
        pthread_mutex_lock(&mutex_rec[2]);
        Editor[2].GetData(received_buf2);//处理数据的时候不要接收
        pthread_mutex_unlock(&mutex_rec[2]);
        if(Editor[2].validData==false)
        {
            cout<<"Data from Robot 2 wrong,please enter again"<<endl;
            receivedFlag[2]=false;
            continue;
        }
        else
        {
            receivedFlag[2]=true;
            if (Editor[2].isRobotOnline() ) //&& Editor[1].PrintRobotNumber() == 1
            {
                robot[2].isRobotOnline=true;
                if (Editor[2].PrintWorkingState() == Initial)//在线状态0
                {
                    //先让当前位置无效化，再计算，最后在发送的时候判断位置是否有效，有效才发
                    tmpPos[2]=65535;
                    tmpPos[2] = robot[2].PrintNearestNode(Editor[2].PrintRobotPosX(), Editor[2].PrintRobotPosY());
                    robot[2].SetCurrentPos(tmpPos[2]);
                }
                else if (Editor[2].PrintWorkingState() == RequestPath)//在线状态1
                {
                    //pthread_mutex_lock(&mutex_rec);
                    robot[2].LocalPaths.clear();
                    robot[2].SetGoal(Editor[2].PrintRobotFinalGoal());
                    tmpPos[2] = Editor[2].PrintRobotCurrentNode();
                    if(tmpPos[2]>robot[2].PrintMaxNode())      //加入一个位置重计算功能，当机器人的位置越出地图最大节点数目时，认为某些地方出错，则机器人的位置由服务器重新计算
                        tmpPos[2]=robot[2].PrintNearestNode(Editor[2].PrintRobotPosX(),Editor[2].PrintRobotPosY());
                    robot[2].SetCurrentPos(tmpPos[2]);
                    robot[2].OtherS.clear();
                    robot[2].clearAllEdges();
                    /*
                    robot[1].UpdateOccupiedNodesOfOtherRobots(robot[0].LocalS);
                    robot[1].UpdateOccupiedNodesOfOtherRobots(robot[2].LocalS);
                    robot[1].UpdateOccupiedNodesOfOtherRobots(robot[3].LocalS);
                    ...
                    for (int m = 0; m < 3; m++)
                    {
                        ...
                        {
                            robot[2].UpdateInverseEdges(robot[m].LocalPaths);
                        }
                    }
                    ...
                    //robot[1].UpdateInverseEdges(robot[3].LocalPaths);
                    //搜索路径
                    robot[2].DijkstraSP(tmpPos[2]);
                    if(robot[2].LocalPaths.empty())
                        robot[2].LocalPaths.push_back(tmpPos[2]);

                } else if (Editor[2].PrintWorkingState() == Pending)//在线状态2
                {
                    //机器人正在行进，跳过
                    cout << "Robot is pending" << endl;
                } else if (Editor[2].PrintWorkingState() == ReachFinalGoal)//在线状态3
                {
                    //机器人到达全局路径终点
                    cout << "Robot has arrived the final goal" << endl;
                } else {
        ...
        if(nodeAlloc.OriginalNodes.size()>=3)//三台机器人的位置都连上了才继续执行后面的内容，否则直接返回
        ...
        for (int j = 0; j <nodeAlloc.AdjustedNodes.size() ; j++)//nodeAlloc.AdjustedNodes.size()
        {
            ...
            if(Editor[j].m_subString.size()<=5)//再次确保数据是有效的
            {
                continue;
            }
    ...
    while(1)
    {
        ...
        */
        ...
        if(strncmp(buf, "exit", 4) == 0 )
        {
            ...
            pthread_cancel(SearchThread);
            pthread_cancel(display_thread);//退出显示线程
            pthread_cancel(send_thread3);//退出发送线程
            break;//退出
        }
...
void * send_msg_2(void *arg)
{
    pthread_mutex_t mutex_send2;
    char buf[1024] = {0};
    memset(send_buf[2],0,1024);
    //strcpy(send_buf2,"-1;2;1;1;0;13;1.1;1.2;1.5;2.1;3;5;7;10;");
    int writeFlag;
    //strcpy(send_buf2,"3;|;1;0.1;0.1;1.1;1.1;3;| 2;0.2;0.2;2.2;2.2;3;| #");
    //strcpy(buf,"0.5;-0.5;1.0;1.0;-2;0.5;1;7;8;9;10;11;-1;-2;-3;");
    ...
    while(1)
    {
        ...
        //strcpy(send_buf[1],"0 1|1 0.1 0.1 1.1 1.1 3|2 0.2 0.2 2.2 2.2 3|#");
        //写入节点的处理语句
        strcpy(buf,send_buf[2]);

        sleep(1);
        pthread_mutex_lock(&mutex_send[2]);
        writeFlag=write(*socket_fd,buf,strlen(buf));
        pthread_mutex_unlock(&mutex_send[2]);
        if(writeFlag<=0)
            continue;
        sleep(1);
        cout<<"2号机器人发送:";
