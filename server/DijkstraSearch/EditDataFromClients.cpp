...
bool EditDataFromClients::my_SubString()//返回N串以;分割的字串，每串都是string类型,同时s也被赋值
{
    ...
    if(ss.size()>=2)
    {
        vector<string>copy_ss;
        copy_ss.clear();
        copy_ss=split(ss.at(1),pattern);
        m_subString.clear();
        m_subString=split(ss.at(1),pattern);
...
// cout<<"Data invalid"<<endl;
    ...
    for(int i=0;i<m_subString.size()-1;i++)
    {
        ClientData.push_back(atof(m_subString[i].data()));
    }
