...
{
    img0=image;
    Xworld_Origin=-12.2;//-12.2 //Gazebo仿真下是-10.0 -10.0 实验室地图是-12.2 -15.4
    Yworld_Origin=-13.8;//-15.4
...
void Robot::AdjacentDiffusion( int selectedCol,int expansionRatio)
{
    ...
    int distance=14;//这里的distance需要特别注意，是两个节点的中心的间距，如果错了就无法定位到末端节点，
    int initialRow=4+selectedCol % distance, initialCol = 4;
